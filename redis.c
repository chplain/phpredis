...
static zend_function_entry redis_functions[] = {
     ...
     PHP_ME(Redis, rlGetSet, NULL, ZEND_ACC_PUBLIC)
     ...
     PHP_ME(Redis, rlHGetSet, NULL, ZEND_ACC_PUBLIC)
...
PHP_METHOD(Redis, rlGet)
{
    ...
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "RL_GET", "s", key, key_len);
    if(key_free) efree(key);
...
PHP_METHOD(Redis, rlGetSet)
{
    ...
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "RL_GETSET", "s", key, key_len);
    if(key_free) efree(key);
...
PHP_METHOD(Redis, rlSet)
{
    ...
    val_free = redis_serialize(redis_sock, z_value, &val, &val_len TSRMLS_CC);
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "RL_SET", "ss", key, key_len, val, val_len);
    if(val_free) efree(val);
    if(key_free) efree(key);
...
PHP_METHOD(Redis, dsAppend)
{
    ...
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "DS_APPEND", "ss", key, key_len, val, val_len);
    if(key_free) efree(key);
...
PHP_METHOD(Redis, dsHGet)
{
    ...
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "DS_HGET", "ss", key, key_len, member, member_len);
    if(key_free) efree(key);
...
PHP_METHOD(Redis, rlHGet)
{
    ...
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "RL_HGET", "ss", key, key_len, member, member_len);
    if(key_free) efree(key);
...
PHP_METHOD(Redis, rlHGetSet)
{
    ...
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "RL_HGETSET", "ss", key, key_len, member, member_len);
    if(key_free) efree(key);
...
{
    generic_hset(INTERNAL_FUNCTION_PARAM_PASSTHRU, "DS_HSET", redis_long_response);
}

PHP_METHOD(Redis, rlHSet)
{
    generic_hset(INTERNAL_FUNCTION_PARAM_PASSTHRU, "RL_HSET", redis_long_response);
}
...
PHP_METHOD(Redis, dsHIncrBy)
{
    ...
    /* HINCRBY key member amount */
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "DS_HINCRBY", "sss", key, key_len, member, member_len, val, val_len);
    if(key_free) efree(key);
...
PHP_METHOD(Redis, dsHGetAll) {
    ...
    RedisSock *redis_sock = generic_hash_command_1(INTERNAL_FUNCTION_PARAM_PASSTHRU, "DS_HGETALL", sizeof("DS_HGETALL")-1);
    if(!redis_sock)
        RETURN_FALSE;
...
PHP_METHOD(Redis, dsExists)
{
    ...
    key_free = redis_key_prefix(redis_sock, &key, &key_len TSRMLS_CC);
    cmd_len = redis_cmd_format_static(&cmd, "DS_EXISTS", "s", key, key_len);
    if(key_free) efree(key);
...
PHP_METHOD(Redis, dsHExists)
{
    char *cmd;
    int cmd_len;
    RedisSock *redis_sock = generic_hash_command_2(INTERNAL_FUNCTION_PARAM_PASSTHRU, "DS_HEXISTS", 10, &cmd, &cmd_len);
    if(!redis_sock)
        RETURN_FALSE;
...
{
    redis_generic_setex(INTERNAL_FUNCTION_PARAM_PASSTHRU, "SETNEX");
}
